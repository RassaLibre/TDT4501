...
App.UIC.Panel = React.createClass({
    ...
    */
    render: function() {
        ...
        var decision = "";
        if(this.state.show_json){
            decision = (<textarea className="json_text_area" rows="10" value={this.state.json} readOnly/>);
        }
        else{
            decision = (
        ...
        return(
            <div className="container" style={this.state.styles}>
                <h1 className="move_panel_button">
                    <span onClick={this.move} className={move_control_panel_icon_classes}></span>
                </h1>
                <h1>Control panel</h1>
                <button className="btn" onClick={this.generate_json}>{generate_button_title}</button>
                {decision}
            </div>
        );
    }
